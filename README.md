...
- `infra/docker-compose.yml`: RabbitMQ plus optional worker containers
...
8. Start the long-running local workers:
   - `pnpm workers:up`
9. Stop the worker containers when needed:
   - `pnpm workers:down`
10. Run one-shot worker iterations manually when needed:
   - `pnpm queue:publish`
   - `pnpm queue:release`
...
- RabbitMQ-backed outbox worker and priority-chain release worker
